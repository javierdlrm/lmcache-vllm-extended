...
from lmcache_vllm.vllm_adapter import (
    ...
    lmcache_blend_drop_spt,
    lmcache_remove_request_id_indices,
    StoreStatus,
    RetrieveStatus,
    SUPPORTED_MODELS,
)
...
def new_execute_model(
    ...
    # CacheBlend updates
    if (
        lmcache_get_config().enable_blending
        and hasattr(model_input.attn_metadata, "blend_metadata")
        and model_input.attn_metadata.blend_metadata.selected_token_indices is not None
    ):
        ...
        logger.debug(
            f"Updating selected_token_indices to {new_selected_token_indices} after blending"
        )
...
def new_log_task_completion(task: asyncio.Task, error_callback) -> None:
...
def wrap_prepare_model_input(
    ...
    return dataclasses.replace(
        model_input, seq_group_metadata_list=seq_group_metadata_list
    )
...
def wrap_prepare_model_input_tensors(
    self, seq_group_metadata_list, finished_requests_ids: Optional[List[str]] = None
):
    model_input = original_prepare_model_input_tensors(
        self, seq_group_metadata_list, finished_requests_ids
    )
    attn_metadata = model_input.attn_metadata
...
def new_asdict_zerocopy(self, skip_fields: Optional[Set[str]] = None) -> Dict[str, Any]:
...
def new_from_broadcasted_tensor_dict_with_sampling(
    ...
    if "blend_metadata" in tensor_dict:
        ...
        setattr(
            tensor_dict["attn_metadata"],
            "blend_metadata",
            tensor_dict["blend_metadata"],
        )
...
def new_extract_prompt_components(self, inputs, request_id, lora_request=None):
    prompt, prompt_token_ids, multi_modal_data = original_extract_prompt_components(
...
async def new_extract_prompt_components_async(
    ...
    prompt, prompt_token_ids, multi_modal_data = (
        await original_extract_prompt_components_async(
            self, inputs, request_id, lora_request
        )
    )
    prompt_token_ids = lmcache_blend_drop_spt(request_id, prompt_token_ids)
...
def inject_blend():
    ...
    vllm.attention.backends.abstract.AttentionMetadata.asdict_zerocopy = (
        new_asdict_zerocopy
    )
    ...
    original_extract_prompt_components = (
        vllm.inputs.preprocess.InputPreprocessor._extract_prompt_components
    )
    original_extract_prompt_components_async = (
        vllm.inputs.preprocess.InputPreprocessor._extract_prompt_components_async
    )
    vllm.inputs.preprocess.InputPreprocessor._extract_prompt_components = (
        new_extract_prompt_components
    )
    vllm.inputs.preprocess.InputPreprocessor._extract_prompt_components_async = (
        new_extract_prompt_components_async
    )
...
def InitLMCacheEnvironment() -> None:
    ...
    original_prepare_model_input_tensors = (
        vllm.worker.model_runner.ModelRunner._prepare_model_input_tensors
    )
    vllm.worker.model_runner.ModelRunner._prepare_model_input_tensors = (
        wrap_prepare_model_input_tensors
    )
    ...
    vllm.inputs.preprocess.InputPreprocessor._tokenize_prompt_async = (
        _new_tokenize_prompt_async
    )
